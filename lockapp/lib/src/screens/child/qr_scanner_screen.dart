...
import 'package:lockapp/features/auth/data/models/register_request.dart';
import 'package:lockapp/features/auth/data/models/login_request_model.dart';
...
class _QrScannerScreenState extends ConsumerState<QrScannerScreen> {
  ...
  Future<void> _processQrCode(String code) async {
    ...
    try {
      ...
      // 2. Auto-Register the child
      final uniqueId = DateTime.now().millisecondsSinceEpoch.toString();
      final tempEmail = 'child_$uniqueId@lockapp.local';
      final tempPassword = 'pass_$uniqueId'; // Securely generate this in real app

      try {
        final authRepo = ref.read(authRepositoryProvider);
        
        // Register
        await authRepo.register(RegisterRequest(
          email: tempEmail,
          password: tempPassword,
          role: 'Child',
          deviceId: 'child_device_$uniqueId',
        ));

        // Login to get the User ID/Token
        final user = await authRepo.login(LoginRequestModel(
          email: tempEmail,
          password: tempPassword,
        ));
        
        // 3. Process the pairing request with the new authenticated user
        // Note: The original acceptPairingRequest in service might need updating 
        // if it mocks the user creation. Since we are using real backend auth now,
        // we should pass the real user ID to the service or let the service use the current user.
        
        // For now, we'll try to use the existing service method.
        // If the service is MOCKING data, it might ignore our real user.
        // We will assume for this step we want to call the backend pairing endpoint.
        // But since QrPairingService is currently MOCKING, we will just call it as is
        // and let it do its mock logic, or we would need to rewrite QrPairingService to use the API.
        
        // Let's stick to the current service call which returns a user (mock).
        // In a real scenario, we would call:
        // await _qrService.pairDevice(code, user.id, deviceId);
        ...
        if (result != null) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Cihaz başarıyla eşleştirildi ve kaydedildi.')),
            );
        ...
        // Auth or Pairing failed
         if (mounted) {
          _showErrorDialog('Kayıt/Eşleştirme hatası: ${e.toString()}');
        }
      }

    } catch (e) {
      if (mounted) {
        _showErrorDialog('Beklenmeyen hata: ${e.toString()}');
      }
    } finally {
      if (mounted) {
        setState(() => _isProcessing = false);
