...
import 'package:flutter/services.dart'; // For MethodChannel, though mocked
import 'dart:async';

class AppBlockingService {
  static const MethodChannel _channel = MethodChannel('lockapp/app_blocking'); // Still needed for method signatures

  // In-memory storage for mock data
  static final List<AppBlockRule> _mockBlockRules = [];
  ...
  Future<List<Map<String, dynamic>>> getInstalledApps() async {
    ...
    return [
      {'packageName': 'com.instagram.android', 'appName': 'Instagram', 'appIcon': 'base64icon1'},
      {'packageName': 'com.facebook.katana', 'appName': 'Facebook', 'appIcon': 'base64icon2'},
      {'packageName': 'com.whatsapp', 'appName': 'WhatsApp', 'appIcon': 'base64icon3'},
      {'packageName': 'com.google.android.youtube', 'appName': 'YouTube', 'appIcon': 'base64icon4'},
      {'packageName': 'com.spotify.music', 'appName': 'Spotify', 'appIcon': 'base64icon5'},
    ];
  }
  ...
  Future<List<AppBlockRule>> getBlockRules({
    ...
    await Future.delayed(const Duration(milliseconds: 300));
    return _mockBlockRules.where((rule) => rule.childUserId == childUserId).toList();
  }
  ...
  Future<AppBlockRule> updateBlockRule({
    ...
    if (index != -1) {
      final updatedRule = _mockBlockRules[index].copyWith(
        isBlocked: isBlocked,
        reason: reason,
        blockedUntil: blockedUntil,
        updatedAt: DateTime.now(),
  ...
  Future<List<BlockAttemptLog>> getBlockAttemptLogs({
    ...
    return _mockBlockAttemptLogs.where((log) {
      bool matches = log.childUserId == childUserId;
      if (startDate != null) {
        matches = matches && log.attemptTime.isAfter(startDate);
      }
      if (endDate != null) {
        matches = matches && log.attemptTime.isBefore(endDate);
      }
      return matches;
    }).toList();
  }
  ...
  Future<AppControlSummary> generateAppControlSummary({
    ...
    await Future.delayed(const Duration(milliseconds: 500));
    final blockedApps = _mockBlockRules.where((rule) => rule.childUserId == childUserId && rule.isCurrentlyBlocked).toList();
    final blockAttempts = _mockBlockAttemptLogs.where((log) => log.childUserId == childUserId && log.attemptTime.day == date.day).toList();
